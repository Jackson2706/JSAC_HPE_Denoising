...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 22 23:51:20 2024
...
def apply_kalman_filter_4d(csi_data, process_variance=1e-5, measurement_variance=1e-2):
    """
    Apply Kalman Filter to 4D WiFi CSI data for denoising.
    ...
    Returns:
        numpy array: Denoised CSI data after applying Kalman Filter.
    """
    ...
    # Apply Kalman Filter over time series for each batch, channel, and subcarrier
    for b in range(batch_size):
        for channel in range(num_channels):  # Loop through transmitters * receivers
            for subcarrier in range(num_subcarriers):  # Loop through subcarriers
                # Initialize Kalman Filter for each time series
                kf = KalmanFilter(process_variance, measurement_variance, initial_estimate=csi_data[b, channel, subcarrier, 0])
                for t in range(num_timesteps):  # Loop through the time steps
                    denoised_csi_data[b, channel, subcarrier, t] = kf.update(csi_data[b, channel, subcarrier, t])
